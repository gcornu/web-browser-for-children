...
function createPageMods() {
	...
	if(ss.storage.customWhitelist && ss.storage.customWhitelist.length) {
		whitelistString += '|' + ss.storage.customWhitelist.join('|');
	} else {
...
function addElementsToDefaultWhitelist(elements, category) {
	elements.forEach(function(element) {
		...
		if(index !== -1) {
			...
			defaultWhitelists.push(element);
		} else {
	...
	elements.forEach(function(element) {
		if(ss.storage.removedDefaultWhitelistElements[category].indexOf(element) === -1) {
			...
			if(index !== -1) {
				...
				console.log('Host is not in the default whitelist');
			}
		} else {
...
function removeElementsFromCustomWhitelist(elements, category) {
	elements.forEach(function(element) {
		if(ss.storage.customWhitelist[category]) {
			...
			console.log('The given category contains nothing');
		}
	});
}
