...
function toggle(boolvalue) {
	...
	if(boolvalue) {
		// make the elements visible even in full screen
		user_autohide_pref = prefs.get('browser.fullscreen.autohide');
		prefs.set('browser.fullscreen.autohide', false);
		...
		// restore user value
		prefs.set('browser.fullscreen.autohide', user_autohide_pref);
...
function removeAll() {
	...
	var buttonsToKeepIds = new Array('nav-bar-customization-target', 'nav-bar-overflow-button');
	...
	//removing useless buttons in nav-bar-customization-target and storing them
	...
	var buttons = document.getElementById('nav-bar-customization-target').childNodes;
	//getting the IDs of these buttons - seems useless to do so (you get the element, then the id and finally get the id again), but if I don't it doesn't work properly : some unrelevant buttons are kept alive
	var buttonsIds = new Array();
	for(var i=0; i<buttons.length; i++) {
		buttonsIds.push(buttons[i].getAttribute('id'));
