...
function init(enab) {
	...
	if(!initialized) {
		...
		fileHandler.readFile('blacklist.txt', callback);
	} else if(!enabling) {
...
function callback(data) {
	console.log('Got response, beggining treatment');
	...
	splitArray.forEach(function (element) {
		if(element.charAt(0) == '[' && element.charAt(element.length-1) == ']') {
			...
			if(ss.storage.removedDefaultBlacklistElements[currentCategory].indexOf(element) === -1) {
...
function createPageMods() {
	...
	pageMod.PageMod({
		...
		onAttach: function (worker) {
			...
			if(host !== oldHost) {
				if(isInCustomBlacklist(wildcardedHost)) {
					console.log('host in custom blacklist');
					denyPage(worker, true);
					oldHostBlacklisted = true;
				} else {
					var blacklisted = isInDefaultBlacklist(host);
					console.log('isInDefaultBlacklist ' + blacklisted);
					denyPage(worker, blacklisted);
					oldHostBlacklisted = blacklisted;
				}
				oldHost = host;
			} else {
...
/**
 * Test if the given host is in the custom blacklist or not
 *
 * @param {String} the host to be tested
 */
function isInCustomBlacklist(host) {
	var BreakException= {};
	try {
		var customBlacklist = ss.storage.customBlacklist;
	    Object.keys(customBlacklist).forEach(function (category) {
			if(customBlacklist[category].indexOf(host) !== -1) {
...
/**
 * Test if the given host is in the default blacklist or not
 *
 * @param {String} the host to be tested
 */
function isInDefaultBlacklist(host) {
	var BreakException= {};
	try {
	    Object.keys(defaultBlacklists).forEach(function (category) {
	    	defaultBlacklists[category].forEach(function (blacklist) {
