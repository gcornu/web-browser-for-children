...
exports.addCustomBlacklistCategory = addCustomBlacklistCategory;
exports.removeCustomBlacklistCategory = removeCustomBlacklistCategory;
...
function createPageMods() {
	...
	blacklistPageMod = pageMod.PageMod({
		...
		contentScriptOptions: {
			...
			not_allowed_title: _('blacklisted-not-allowed-title'),
		},
		contentScriptWhen: 'start',
		onAttach: function (worker) {
			var host = url.URL(worker.tab.url).host;
			//var wildcardedHost = host.replace(/^(\w+)\.(\w+)/, '*.$2');
			var wildcardedHost = utils.isSecondLevelDomain(/^(\w+)\.(\w+\.\w+)$/.exec(host)[2]) ? '*.' + host : host.replace(/^(\w+)\.(\w+)/, '*.$2');
			console.log(wildcardedHost);
...
function addElementToCustomBlacklist(element, category) {
	if(category !== null) {
		...
		if(/^(\w+)\.(\w+)\.(\w+)/.test(host)) {
			if(!utils.isSecondLevelDomain(/^(\w+)\.(\w+\.\w+)$/.exec(host)[2])) {
...
/**
 * Add the given category to the custom blacklist
 *
 * @param {string} the category to add
 */
function addCustomBlacklistCategory(category) {
	if(!ss.storage.customBlacklist[category]) {
		ss.storage.customBlacklist[category] = [];
	} else {
...
/**
 * Remove the given category from the custom blacklist
 *
 * @param {string} name of the category to be deleted
 */
function removeCustomBlacklistCategory(category) {
	if(ss.storage.customBlacklist[category]) {
		delete ss.storage.customBlacklist[category];
	} else {
		throw new Error('The given category doesn\'t exists');
	}
