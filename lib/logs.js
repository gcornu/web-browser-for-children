...
exports.historyListener = historyListener;
...
function setHistoryCallback(cbParam) {
	historyCallback = cbParam;
}
...
function historyListener(tab) { //register logging event on tab load
	fileHandler.readJSON('history.json', function(data) { //get old history file
		...
		fileHandler.writeJSON(data, 'history.json'); //overwrite history file with new data
		if(historyCallback) {
			historyCallback();
		}
		//tabs.emit("loggedVisit");
	});
}
