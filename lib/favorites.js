		...
		onAttach: function(worker) {
			...
			worker.port.on('add_favorite' , function(url_to_add){
				function callback_add_favorite(favorites_file){
					...
					//the 2 images that can be sent as result to the fav_toolbar
					...
					if(favorites.length >= max_fav){
						worker.port.emit('favorite_answer' , favorite_rejected);
					}
					else{
						...
						if(already_favorite){
							...
							worker.port.emit('favorite_answer' , favorite_accepted);
						}
					}
				}
