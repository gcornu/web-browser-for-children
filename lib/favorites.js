...
function setFav_toolbar(){
	...
	var frame = new Frame({
		url: "./panels/fav-toolbar.html",
		onMessage: function (e) {
			switch(e.data.type) {
				case 'init': 
					//these lines send the content of favorites file to fav_toolbar
					function callback_send_favorites (favorites_file) {
						var data_to_send = { 'star': star_icon , 'delete_icon': delete_icon , 'favorites': favorites_file.favorites};
						favorites_file.favorites.forEach(function (favorite) {
							e.source.postMessage(favorite, e.origin);
						});
					}
					fileHandler.readJSON('child-favorites.json', callback_send_favorites);
					break;
				case 'fav_element_clicked':
					tabs.activeTab.url = e.data.href;
					break;
				case 'add_favorite':
					function callback_add_favorite (favorites_file) {
						//getting the favorites array
						...
						//getting the current URI and title
						var url_to_add = utils.getActiveURI();
						var title_to_add = tabs.activeTab.title;
						...
						if(favorites.length >= max_fav) {
							...
							if(already_favorite) {
								...
								//if url not in favorites : add it and rewrite the favorite file (as the nb of favorites is very limited, we can afford to rewrite whole file everytime)
								var new_favorite = {'index': max_id+1, 'title': title_to_add, 'uri': url_to_add};			
								favorites.push(new_favorite);
								var file_content = {'favorites': favorites};					
								fileHandler.writeJSON(file_content, 'child-favorites.json'); 
								
								e.source.postMessage(new_favorite, e.origin);
							}
						}
					}
					fileHandler.readJSON('child-favorites.json', callback_add_favorite);
