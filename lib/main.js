...
function main(options) {
	...
	pageMod.PageMod({
		...
		contentScriptOptions: {
			...
			time_limit_method_set: _('settings-limit-time-method-set'),
...
var parentUI = (function () { //set up all the necessary panels in a "parentUI" object
	...
	add_list.blacklist = panels.Panel({
		...
		contentScriptFile: [data.url("jquery-2.0.3.js"), data.url('js/bootstrap.min.js'), data.url('js/bootstrap-select.min.js'), data.url("panels/addList.js")],
		contentScriptOptions: {'listType': 'blacklist'}
	});
	...
	add_list.whitelist = panels.Panel({
		...
		contentURL: data.url('panels/addList.html'),
		contentScriptFile: [data.url("jquery-2.0.3.js"), data.url('js/bootstrap.min.js'), data.url('js/bootstrap-select.min.js'), data.url("panels/addList.js")],
		contentScriptOptions: {'listType': 'whitelist'}
	});
...
function attachSettingsListeners(worker) {
	...
	worker.port.on('add_custom_whitelist_category', function (category) {
		blacklisting.addCustomWhitelistCategory(category);
	});
